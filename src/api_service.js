...
export default new class{
    ...
    get_profile_posts(user){
        axios.get(baseURL+'profile_posts/'+user).then(({data})=>{
            return data.data
        })
    }

    get_user_profile(user){
        axios.get(baseURL+'user/profile/'+user).then(({data})=>{
            return data
        })
    }
    ...
    get_tags(){
        axios.get(baseURL+'tags').then(({data})=>{
            return data.data
        })
    }

    get_notifications(user){
        axios.get(baseURL+'notification/user/'+user.id).then(({data})=>{
            return data.data
        })
    }
